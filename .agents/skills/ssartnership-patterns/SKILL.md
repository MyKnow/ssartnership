...
- The project `prepush` script aliases `verify:change`, which classifies the actual merge-base diff through the same repository module as GitHub. `docs` skips runtime checks, `development` runs changed-file lint and semantic typecheck, `ui` adds Node/unit tests, and `standard`/`high` run the complete Quick profile. Unknown paths, deletions, renames, oversized changes, CI, dependencies, migrations, API/actions, auth/security, repositories, and E2E contracts fail closed to `high`. `verify:release` remains the complete Production build plus retry-free full E2E suite for `dev` to `main` promotion. Actions and Vercel must use `npm run install:trusted`; raw dependency materialization with `npm ci`/`npm install` is forbidden. The trusted path disables every lifecycle, requires exact registry URLs plus SHA-512 lock integrity, never runs `esbuild/install.js`, and executes an `esbuild` binary only after platform-specific SHA-256 verification. Because the canonical release script pushes with `--no-verify`, `scripts/release.mjs` calls `npm run prepush` explicitly before branch and main/tag pushes.
...
- Preview data sync is a manual maintenance operation bound to the exact live `dev` SHA. Only `dev` pushes that change `supabase/migrations/**` or `supabase/schema.sql` start automatic Preview migration application; ordinary pushes create no migration run and never copy Production data or Storage. For a manual sync failure, inspect sanitizer diagnostics and missing relation errors first.
- Main promotion: create a short-lived `dev` to `main` PR only when Production is ready. Require the always-running `CI Policy Gate` and Vercel before merge; after merge, monitor the exact Production SHA's policy gate and Vercel deployment. Never require a conditionally skipped verification job directly in branch protection.
