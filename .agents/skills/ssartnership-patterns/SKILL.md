...
- Broad UI waves and Production promotion: focused E2E is insufficient. Run `npm run verify:release` or wait for the `Release Readiness` job on the `dev` to `main` PR. Update assertions when the UI contract intentionally changes, use accessible step names, derive responsive columns from computed CSS instead of fixture cardinality, and give first-compile redirects an explicit timeout.
...
- Preview data sync is a manual maintenance operation bound to the exact live `dev` SHA. Normal `dev` pushes apply pending Preview migrations after Quick Readiness without copying Production data or Storage. For a manual sync failure, inspect sanitizer diagnostics and missing relation errors first.
- Main promotion: create a short-lived `dev` to `main` PR only when Production is ready. Require `Quick Readiness`, `Release Readiness`, and Vercel before merge; after merge, monitor the exact Production SHA's Quick Readiness and Vercel deployment.
