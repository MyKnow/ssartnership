...
- The project `prepush` script is the required local Public Readiness gate: it verifies the trusted-install policy before `npm run check:lockfile` and finishes with `CI=1 PLAYWRIGHT_CHROMIUM_CHANNEL=chrome playwright test`. Actions and Vercel must use `npm run install:trusted`; raw dependency materialization with `npm ci`/`npm install` is forbidden. The trusted path disables every lifecycle, requires exact registry URLs plus SHA-512 lock integrity, never runs `esbuild/install.js`, and executes an `esbuild` binary only after platform-specific SHA-256 verification. The ECC global pre-push hook executes this optional repository gate after lint/test/build. Because the canonical release script pushes with `--no-verify`, `scripts/release.sh` must also call `npm run prepush` explicitly before branch and main/tag pushes.
