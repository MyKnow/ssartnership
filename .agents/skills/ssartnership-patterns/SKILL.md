...
- Do not reload immediately after a cookie-setting login server action. Under first-compile CI timing, allow the dynamic chooser up to 30 seconds and give the whole flow a 60-second timeout so the transition cookie is preserved.
