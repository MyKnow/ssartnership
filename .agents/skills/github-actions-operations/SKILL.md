...
3. Resolve the exact repository, target branch, head SHA, event, expected workflows, and any moving PR that will also run against the same SHA.
...
- Never materialize dependencies with raw `npm ci` or `npm install` in Actions or Vercel. Use `npm run install:trusted`: it verifies the static/effective npm policy and every reviewed non-registry dependency, installs with every lifecycle script disabled after application/provider secrets are scrubbed (reviewed proxy/CA transport variables may remain), then verifies and executes the integrity-pinned platform `esbuild@0.28.1` binary directly. It never runs `esbuild/install.js` or another lifecycle installer. Keep `.npmrc` at `ignore-scripts=true`, `strict-allow-scripts=true`, `dangerously-allow-all-scripts=false`, and `omit-lockfile-registry-resolved=false`. The sole reviewed raw-install exception is the post-install, `--package-lock-only --ignore-scripts` canonical lockfile verification; it may not materialize dependencies. This explicit path is required because npm's native allow-script preflight can see git/directory `prepare` only after package extraction has already run it.
...
- Run `npm run check:lockfile` and `npm run prepush`. Preserve exact Node/npm on GitHub; Vercel and local install policy currently accepts reviewed npm 11.16+ but rejects npm 12 until separately reviewed.
