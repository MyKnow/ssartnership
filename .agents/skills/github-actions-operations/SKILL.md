...
- In the explicit non-Production Playwright mock runtime, persistence-only telemetry must use the repository's reviewed no-op boundary instead of attempting Production/Preview clients and emitting expected errors. After a form or URL-backed navigation assertion, wait for the response lifecycle to settle before the test ends; a changed URL alone does not prove the server stream completed.
