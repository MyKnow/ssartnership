...
- In the explicit non-Production Playwright mock runtime, persistence-only telemetry must use the repository's reviewed no-op boundary instead of attempting Production/Preview clients and emitting expected errors. After a form, URL-backed navigation, or render that starts late link-prefetch/stream work, place the final network-idle wait after that flow's last meaningful assertion or capture; a URL change or an idle event reached earlier in the test does not prove teardown is safe. Do not add a suite-wide request-drain `afterEach`: development-server, stream, or unrelated requests can turn that broad cleanup into a new timeout.
