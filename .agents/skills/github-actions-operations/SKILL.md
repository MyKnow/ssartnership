...
- Error-like words inside a structurally proven passed test-result line are not runtime errors. Suppress only the exact reviewed test-result form and only the ambiguous `timeout`/`deprecation` signatures; `##[error]`, `##[warning]`, retries, failures, transport resets, and provider diagnostics remain reportable regardless of nearby test text.
