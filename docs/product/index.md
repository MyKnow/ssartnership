---
title: 제품 지식 인덱스
type: index
status: current
authority: normative
last_verified: 2026-08-29
---

# 제품 지식 인덱스

제품 문서는 사용자에게 어떤 가치와 동작을 제공해야 하는지 설명한다. 실제 route, repository, schema의 현재 구현은 코드와 테스트에서 확인한다.

## 핵심 기준

- [서비스 개요](./overview.md)
- [정보 구조와 라우트 맵](./information-architecture.md)
- [사용자 흐름](./user-flows.md)
- [기능 인벤토리](./feature-inventory.md)
- [기능 유지·통합·제거 점수표](./feature-scoring.md)
- [제품 용어](./glossary.md)

## 화면 계약

- [화면 계약 인덱스](./screen-specs/index.md)
- [공개·회원 화면](./screen-specs/public-and-member.md)
- [인증 화면](./screen-specs/auth.md)
- [수료생 인증 화면](./screen-specs/graduate-verification.md)
- [프로젝트 쇼케이스 이벤트 화면](./screen-specs/project-showcase.md)
- [파트너 포털](./screen-specs/partner.md)
- [관리자 화면](./screen-specs/admin.md)

## 운영 가이드

- [카페 싸피 PIN 혜택 촬영](./guides/cafe-ssafy-pin-demo-filming.md)
- [제휴처 쿠폰 운영](./guides/partner-coupon-operations.md)

- [접근과 혜택의 공유 정책](./policies/access-and-benefits.md)
