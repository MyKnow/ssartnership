...
- 목표·위계: Mattermost/이메일 인증 수단 선택 → 대상 계정 확인 → 새 비밀번호 설정 순이다.
- 액션·흐름: 운영진·재학생은 대상 기수의 active Sender가 보낸 6자리 DM 코드를 사용한다. Mattermost가 불가해도 자동 이메일 fallback은 하지 않는다. 수료로 Mattermost 로그인이 불가능한 기존 회원은 별도 텍스트 버튼에서 관리자 승인 복구 신청으로 이동한다.
- 경계·상태: 코드는 HMAC hash만 보관하고 10분 만료, 1회 소비, 5회 검증 제한과 재전송 제한을 적용한다. 계정 존재 여부는 응답으로 노출하지 않는다.
- 수용 기준: 재설정 탭은 `Mattermost`와 `이메일` 두 인증 수단만 제공하며 각 탭에는 해당 재설정 입력과 단일 CTA만 노출한다. `수료해서 MM 로그인이 불가능해요` 보조 이동은 두 탭 밖에서 한 번만 노출하고 기존 회원 복구 신청으로 연결한다. 코드 전송 CTA는 마지막 입력과 충분한 간격을 두고 입력 너비에 맞춘다. MM sender가 없거나 실패하면 안전한 일반 오류를 표시한다.
...
- 수용 기준: 직접 링크 호환을 위한 보조 복구 경로이며 비밀번호 확인 단계에서는 정상 로그인 세션을 만들지 않는다. 기본 안내는 기존 비밀번호 로그인 후 필수 이메일 등록 흐름을 사용하고, 이 경로의 만료·코드 오류·중복 이메일은 재시도 가능한 안전한 오류로 표시한다.
