...
- 목표·위계: 운영진·재학생/수료생 유형 선택 → 대상 계정 확인 → 새 비밀번호 설정 → 복구 경로 순이다.
...
- 수용 기준: 재설정 탭은 운영진·재학생과 수료생 두 경로만 제공하며 각 탭에는 해당 재설정 입력과 단일 CTA만 노출한다. 코드 전송 CTA는 마지막 입력과 충분한 간격을 두고 입력 너비에 맞춘다. MM sender가 없거나 실패하면 안전한 일반 오류를 표시한다.
