...
- 액션·흐름: 일반 로그인은 아이디 또는 검증된 이메일과 기존 사이트 비밀번호를 사용한다. 로그인에 성공한 방식만 브라우저에 기억하고 다음 방문의 기본 탭으로 복원한다. 필요 시 강제 비밀번호 변경·정책 동의·원래 `returnTo`로 이어진다.
...
- 수용 기준: 재설정 탭은 운영진·재학생과 수료생 두 경로만 제공한다. MM sender가 없거나 실패하면 안전한 일반 오류를 표시하고, 기존 비밀번호가 있는 회원에게는 이메일 로그인 복구를 안내한다.
