...
- 반응형·분석: 모바일은 하단 탐색을 숨기고 safe area 하단에 sticky action bar 하나만 고정하며, 본문 끝에는 이 CTA에 가려지지 않는 여백을 둔다. 복사 가능한 연락처 정보도 본문에 표시한다. 넓은 화면은 혜택 이용 CTA를 핵심 혜택 카드 최하단의 full-width primary action으로 둔다. 연락처·위치·대상·조건은 `세부 정보` 그룹으로 묶는다. `partner_detail_view`, `benefit_cta_click`, 지도·문의·예약 클릭을 기록한다.
