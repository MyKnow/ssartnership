...
- 반응형·분석: 모바일은 하단 탐색을 숨기고 safe area 하단에 sticky action bar 하나만 고정하며, 본문 끝에는 이 CTA에 가려지지 않는 여백을 둔다. 복사 가능한 연락처 정보도 본문에 표시한다. 상단 식별 카드는 썸네일·메타·고정 크기 제목으로 구성하고, 빈 경우의 일반 설명 문구는 표시하지 않는다. 넓은 화면은 혜택 이용 CTA를 핵심 혜택 카드 최하단의 full-width primary action으로 둔다. 추가 이미지는 모바일에서 대표 이미지 아래의 가로 썸네일 목록을 유지하고, 모든 크기에서 좌우 스와이프로 다음·이전 이미지를 이동한다. 태블릿 이상에서는 선택 이미지를 전체 폭의 중앙 약 65% 크기로 두고, 이전·다음 방향의 최대 세 이미지는 약 70% 이하 크기의 안개 미리보기 더미로 양옆 뒤에 겹쳐 둔다. 카드가 멀수록 더 작고 옅어지며 바깥쪽에 조금만 드러난다. 첫·마지막 이미지에서는 없는 쪽 미리보기와 화살표를 함께 숨긴다. 태블릿 이상의 양끝 화살표는 이동 가능한 쪽에만 보이며 자동 순환하지 않는다. 이용 조건은 핵심 혜택 목록 바로 아래에 같은 섹션 제목과 번호가 있는 inset 목록 템플릿으로 둔다. 연락처·위치·대상은 `세부 정보` 그룹으로 묶는다. `partner_detail_view`, `benefit_cta_click`, 지도·문의·예약 클릭을 기록한다.
