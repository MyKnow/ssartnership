...
- 수용 기준: 대표 이벤트는 캐러셀 위에 별도 가시 제목·설명을 반복하지 않고, 뷰포트 전체 폭의 radius 없는 21:9 직사각형으로 시작한다. 캐러셀 직후에는 알림 설정 배너 없이 `제휴 혜택 찾기`의 짧은 섹션 헤더와 구분선으로 혜택 디렉터리를 시작한다. 검색이 카테고리보다 먼저 오고 설명 없는 compact category chip을 쓴다. Compact에서는 검색·한 줄 카테고리 레일·필터 버튼만 기본 노출하며, 선택된 카테고리는 레일 안에서 자동으로 보이고 검색어·카테고리의 중복 적용 칩은 노출하지 않는다. 캠퍼스·적용 대상은 필터 버튼의 하단 시트에서 임시로 조정한 뒤 `결과 보기`로 적용하며, 정렬은 결과 수와 같은 행에서 바꾼다. 적용된 상세 필터와 결과 수를 닫힌 상태에서도 확인할 수 있어야 하며 URL이 필터의 단일 기준이다. Expanded 이상에서는 검색·카테고리·상세 필터 sidebar를 유지한다. 카드형은 혜택 최대 2개와 `+N`, 대상 요약, 위치, 상세, 즐겨찾기를 유지하고 카드 표면 클릭으로도 상세에 진입한다. 카테고리·즐겨찾기·내부 링크는 각 고유 동작을 유지한다. 리스트형은 모바일·태블릿에서 혜택/대상을 생략하고 Large 데스크톱부터 비교 정보로 노출한다. Compact 리스트는 썸네일·핵심 정보·상세 아이콘을 한 행에 유지하고 44px 조작 영역을 보존한 채 이미지·글자·간격을 축소한다.
