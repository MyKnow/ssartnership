...
- iOS 계열의 glass 표현은 하단 탐색과 모바일 sticky action의 컨테이너에만 제한하고, safe area·본문 하단·toast와 겹치지 않게 배치한다. sticky action 안에서는 primary의 브랜드 대비를 유지한다.
...
- 반응형·분석: 모바일은 하단 탐색을 숨기고 safe area 하단에 sticky action bar 하나만 고정하며, 본문 끝에는 이 CTA에 가려지지 않는 여백을 둔다. 복사 가능한 연락처 정보도 본문에 표시한다. 상단 식별 카드는 모든 화면 폭에서 고정 크기 정사각형 썸네일과 메타·제목을 가로로 배치하고 긴 소개 본문은 넣지 않는다. 넓은 화면은 혜택 이용 CTA를 핵심 혜택 카드 최하단의 full-width primary action으로 둔다. 추가 이미지는 모바일에서 대표 이미지 아래의 가로 썸네일 목록을 유지하고, 모든 크기에서 좌우 스와이프로 다음·이전 이미지를 이동한다. 태블릿 이상에서는 선택 이미지를 전체 폭의 중앙 약 65% 크기로 두고, 이전·다음 방향의 최대 세 이미지는 약 70% 이하 크기의 안개 미리보기 더미로 양옆 뒤에 겹쳐 둔다. 카드가 멀수록 더 작고 옅어지며 바깥쪽에 조금만 드러난다. 첫·마지막 이미지에서는 없는 쪽 미리보기와 화살표를 함께 숨긴다. 태블릿 이상의 양끝 화살표는 이동 가능한 쪽에만 보이며 자동 순환하지 않는다. 이용 조건은 핵심 혜택 목록 바로 아래에 같은 섹션 제목과 번호가 있는 inset 목록 템플릿으로 둔다. 연락처·위치·대상은 `세부 정보` 그룹으로 묶는다. `partner_detail_view`, `benefit_cta_click`, 지도·문의·예약 클릭을 기록한다.
- 수용 기준: 잠긴 혜택은 민감 조건과 URL을 노출하지 않고, CTA는 한 화면에서 중복 primary로 보이지 않는다. 일반 상세 URL에는 목록용 `returnTo`를 노출하지 않으며 브라우저 뒤로가기에서 기존 필터를 복원한다. 공개 혜택은 번호가 있는 핵심 목록과 같은 위계의 이용 조건 목록을 바로 이어 읽히고, 연락처·이용 기간·위치·대상은 한 단계 낮은 정보 그룹으로 둔다. 위치와 적용 대상은 화면 폭과 관계없이 각각 한 행과 독립된 surface로 분리하고 사이에 여백을 둔다. 운영진·교육생·수료생은 항상 함께 표시하되 적용 대상만 강조한다. 소개·태그는 기본 닫힘 disclosure로 유지하고 닫힌 상태에서도 포함 항목을 한 줄로 요약한다. disclosure를 열면 등록된 태그를 생략 없이 모두 표시한다.
