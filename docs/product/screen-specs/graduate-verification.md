...
| 기본 상태 | 3단계 이메일 인증/교육 정보/파일 제출. 현재 단계는 번호와 `aria-current="step"`, 완료 단계는 체크와 완료 텍스트로 구분한다. 제출 후에는 입력 단계 표시를 숨기고 검토 상태를 안내한다. 기수는 교육 시작 연·월로 자동 계산하며 직접 수정할 수 없다. |
...
| 반응형 | 페이지 wrapper가 최대 640px 너비를 소유하며 공용 Card의 최대 너비와 충돌하지 않는다. 320px부터 번호·연결선과 짧은 단계 이름을 3열로 읽을 수 있어야 한다. 이메일 단계 주 액션은 입력창과 같은 너비이고, 이후 단계는 이전 버튼 옆의 남은 폭을 주 액션이 사용한다. 파일 card의 선택 버튼은 좁은 화면에서 다음 행으로 내려가며 파일명은 영역 안에서 줄바꿈한다. 820px 이상에서 시작/종료 입력을 2열로 배치한다. |
...
### Storybook 가입·복구 기수 선택 미리보기

- `EducationDetails`, `FileSubmission`, `CohortRequired`와 복구 대응 Storybook은 `cohortSelectionPreview`를 명시적으로 켜고, 교육 기간 대신 빈 기수 선택을 보여 준다.
- 이 MOCK-ONLY 미리보기의 기수 목록은 현재 자동 계산 가능한 기수까지 최신순으로 보이지만, 선택값을 교육 시작·종료 연월로 변환하거나 실제 신청 요청에 포함하지 않는다. 제출 시에는 접수하지 않음을 안내한다.
- `EducationPeriodDetails`와 실제 화면·API·데이터 계약은 계속 교육 시작 연·월 기반의 자동 기수 계산을 사용한다.
