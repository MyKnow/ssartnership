...
## MobileNav
- `768px` 미만의 공개·회원 화면은 상단 헤더에 브랜드와 알림만 남기고, 핵심 이동은 화면 하단의 공용 탐색으로 제공한다.
...
- 인증이 필요한 쿠폰함·내 정보는 비로그인 상태에서도 같은 위치를 유지하고 로그인 후 원래 목적지로 복귀한다. 인증·복구·검증처럼 집중이 필요한 흐름에서는 하단 탐색을 숨긴다.
