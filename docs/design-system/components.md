...
## MobileNav
- `768px` 미만이라도 화면 하단의 공용 탐색은 홈 화면에서 실행한 standalone PWA에서만 제공한다. Safari·Chrome 같은 일반 모바일 브라우저에서는 브라우저 자체 toolbar와 겹쳐 콘텐츠를 줄이지 않도록 숨기고, 헤더 전체 메뉴에서 홈·검색·쿠폰함·내 정보를 제공한다. 앱 설치는 드로어 안의 별도 섹션을 만들지 않고 헤더의 독립 아이콘으로 제공한다.
- standalone PWA의 상단 헤더에는 브랜드·테마 전환과 알림을 남긴다. 하단 탐색이 보이는 동안 Footer의 테마 전환과 알림센터를 숨겨 같은 동작을 중복하지 않는다.
...
- Android·iOS·iPadOS 일반 브라우저의 새 문서 방문마다 한 번, 상단 헤더 아래의 compact overlay로 앱 설치를 권장한다. `나중에`로 닫은 상태는 현재 문서 방문에서만 유지하며 브라우저 저장소에 영구 기록하지 않는다.
- 설치를 강제하거나 서비스 이용을 막지 않는다. primary는 기기에 맞는 `설치 방법 보기`, secondary는 `나중에`이며 우측 닫기 버튼도 제공한다.
- 인증·복구·설치 안내와 파트너 상세처럼 집중 액션이 있는 경로에서는 노출하지 않는다. 하단 고정 영역을 추가하지 않아 브라우저 toolbar와 본문 CTA를 가리지 않는다.
