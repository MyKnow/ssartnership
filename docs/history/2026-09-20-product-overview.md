---
title: 문서 책임 분리 전 overview 원본
type: history
status: archived
authority: evidence
---

# 문서 책임 분리 전 overview 원본

이 문서는 538b16b4 기준 원문 보존이다. 과거 구현·운영 주장을 현재 상태로 사용하지 않는다. 현행 계약은 [제품 인덱스](../product/index.md)에서 찾는다.
...
- SSAFY Verify runtime reader·writer·route는 제거됐지만 Production에는 레거시 proof 13행, Mattermost alias 8행과 미사용 Vercel env가 남아 있다. 현재 판정은 삭제 보류이며 [레거시 삭제 준비 계획](../plans/active/ssafy-verify-legacy-removal.md)의 승인·함수 교체 순서를 따른다.
