---
title: Mattermost 직접 연동 전환
type: decision
status: current
authority: normative
---

# ADR-0001: Mattermost 직접 연동 전환
...
직접 연동은 외부 runtime 의존을 없애지만, 기수별 Sender credential의 암호화·활성화·rate limit·lifecycle을 서비스가 책임진다. 외부 Verify fallback은 제공하지 않으므로 Mattermost 장애 복구를 별도 흐름으로 유지한다.
...
- SSAFY Verify 위임 유지: 외부 runtime과 운영 의존이 남아 기각했다.
- 직접 연동과 Verify fallback 병행: 두 source of truth와 장애 모드가 생겨 기각했다.
- 이메일만 사용: 재학생·운영진의 기존 Mattermost 소유 확인 계약을 충족하지 못해 기각했다.
