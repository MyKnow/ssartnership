...
create table if not exists partners (
  ...
  created_at timestamp with time zone default now(),
  updated_at timestamp with time zone default now(),
  constraint partners_benefit_verification_pin_check
    check (
  ...
  constraint partners_benefit_use_max_count_check
    check (
      benefit_use_max_count is null
      or benefit_use_max_count between 1 and 2147483647
    ),
  constraint partners_benefit_use_max_count_action_check
    check (benefit_action_type = 'certification' or benefit_use_max_count is null),
...
alter table partners add column if not exists benefit_use_max_count integer;
...
alter table partners drop constraint if exists partners_benefit_use_max_count_check;
alter table partners add constraint partners_benefit_use_max_count_check
  check (benefit_use_max_count is null or benefit_use_max_count between 1 and 2147483647);
alter table partners drop constraint if exists partners_benefit_use_max_count_action_check;
alter table partners add constraint partners_benefit_use_max_count_action_check
  check (benefit_action_type = 'certification' or benefit_use_max_count is null);
...
create table if not exists partner_registration_requests (
  ...
  constraint partner_registration_requests_benefit_use_max_count_action_check
    check (benefit_action_type = 'certification' or benefit_use_max_count is null),
...
alter table partner_registration_requests add column if not exists benefit_use_max_count integer;
...
alter table partner_registration_requests drop constraint if exists partner_registration_requests_benefit_use_max_count_check;
alter table partner_registration_requests add constraint partner_registration_requests_benefit_use_max_count_check
  check (benefit_use_max_count is null or benefit_use_max_count between 1 and 2147483647);
alter table partner_registration_requests drop constraint if exists partner_registration_requests_benefit_use_max_count_action_check;
alter table partner_registration_requests add constraint partner_registration_requests_benefit_use_max_count_action_check
  check (benefit_action_type = 'certification' or benefit_use_max_count is null);
...
create table if not exists partner_benefit_usages (
  id uuid primary key default uuid_generate_v4(),
  partner_id uuid not null references partners(id) on delete cascade,
...
create index if not exists partner_benefit_usages_partner_verified_at_idx
  on partner_benefit_usages(partner_id, verified_at desc);
create index if not exists partner_benefit_usages_member_verified_at_idx
  on partner_benefit_usages(member_id, verified_at desc);
...
create table if not exists public.partner_benefits (
  ...
  created_at timestamp with time zone not null default now(),
  updated_at timestamp with time zone not null default now(),
  constraint partner_benefits_title_length_check check (char_length(trim(title)) between 1 and 500),
  constraint partner_benefits_display_order_check check (display_order >= 0),
  constraint partner_benefits_max_apply_count_check check (max_apply_count is null or max_apply_count between 1 and 2147483647),
...
where not exists (select 1 from public.partner_benefits existing where existing.partner_id = p.id and existing.title = item.title);
update public.partner_benefit_usages usage
set benefit_id = benefit.id
from public.partner_benefits benefit
where usage.benefit_id is null and usage.partner_id = benefit.partner_id and usage.benefit_snapshot = benefit.title;
...
create or replace function public.record_partner_benefit_usage(
  p_partner_id uuid, p_member_id uuid, p_benefit_id uuid, p_use_count integer,
  p_idempotency_key text, p_metadata jsonb default '{}'::jsonb
)
returns table (usage_id uuid, partner_id uuid, member_id uuid, benefit_id uuid, benefit_snapshot text, use_count integer, verified_at timestamp with time zone, created_at timestamp with time zone, is_new boolean)
language plpgsql security definer set search_path = public as $$
declare partner_row public.partners%rowtype; benefit_row public.partner_benefits%rowtype; usage_row public.partner_benefit_usages%rowtype; normalized_key text := trim(coalesce(p_idempotency_key, '')); current_kst_date date := (now() at time zone 'Asia/Seoul')::date; inserted_count integer := 0;
begin
  if p_partner_id is null or p_member_id is null or p_benefit_id is null then raise exception 'partner_benefit_usage_subject_required'; end if;
  if p_use_count is null or p_use_count < 1 then raise exception 'partner_benefit_usage_use_count_invalid'; end if;
  if char_length(normalized_key) < 16 or char_length(normalized_key) > 128 then raise exception 'partner_benefit_usage_idempotency_key_invalid'; end if;
  select * into partner_row from public.partners where id = p_partner_id; if not found then raise exception 'partner_benefit_usage_partner_not_found'; end if;
  if trim(coalesce(partner_row.location, '')) = '온라인' then raise exception 'partner_benefit_usage_online_partner'; end if;
  if partner_row.period_start is not null and partner_row.period_start > current_kst_date then raise exception 'partner_benefit_usage_period_inactive'; end if;
  if partner_row.period_end is not null and partner_row.period_end < current_kst_date then raise exception 'partner_benefit_usage_period_inactive'; end if;
  select * into benefit_row from public.partner_benefits where id = p_benefit_id and partner_id = p_partner_id; if not found then raise exception 'partner_benefit_usage_benefit_not_found'; end if;
  if p_use_count > coalesce(benefit_row.max_apply_count, 1) then raise exception 'partner_benefit_usage_use_count_exceeded'; end if;
  if not exists (select 1 from public.members where id = p_member_id) then raise exception 'partner_benefit_usage_member_not_found'; end if;
  select * into usage_row from public.partner_benefit_usages where idempotency_key = normalized_key for update;
  if found then
    if usage_row.partner_id <> p_partner_id or usage_row.member_id <> p_member_id or usage_row.benefit_id is distinct from p_benefit_id or usage_row.use_count <> p_use_count then raise exception 'partner_benefit_usage_idempotency_conflict'; end if;
    return query select usage_row.id, usage_row.partner_id, usage_row.member_id, usage_row.benefit_id, usage_row.benefit_snapshot, usage_row.use_count, usage_row.verified_at, usage_row.created_at, false; return;
  end if;
  insert into public.partner_benefit_usages (partner_id, member_id, benefit_id, benefit_snapshot, use_count, idempotency_key, metadata) values (p_partner_id, p_member_id, p_benefit_id, benefit_row.title, p_use_count, normalized_key, coalesce(p_metadata, '{}'::jsonb)) on conflict (idempotency_key) do nothing;
  get diagnostics inserted_count = row_count;
  select * into usage_row from public.partner_benefit_usages where idempotency_key = normalized_key for update; if not found then raise exception 'partner_benefit_usage_record_failed'; end if;
  if usage_row.partner_id <> p_partner_id or usage_row.member_id <> p_member_id or usage_row.benefit_id is distinct from p_benefit_id or usage_row.use_count <> p_use_count then raise exception 'partner_benefit_usage_idempotency_conflict'; end if;
  return query select usage_row.id, usage_row.partner_id, usage_row.member_id, usage_row.benefit_id, usage_row.benefit_snapshot, usage_row.use_count, usage_row.verified_at, usage_row.created_at, inserted_count > 0;
end; $$;
...
create or replace function public.update_partner_immediate_fields_with_audit(
  p_partner_id uuid, p_company_ids uuid[], p_thumbnail text, p_images text[], p_tags text[],
  p_benefit_action_type text, p_benefit_action_link text, p_benefit_items jsonb,
...
declare partner_row public.partners%rowtype; item jsonb; title text; benefit_id uuid; requested_ids uuid[] := '{}'; benefit_titles text[] := '{}'; max_count integer; raw_max text;
begin
  ...
  select * into partner_row from public.partners where id = p_partner_id for update; if not found then raise exception 'partner_immediate_update_partner_not_found'; end if;
  if partner_row.company_id is null or not (partner_row.company_id = any(p_company_ids)) or not exists (select 1 from public.partner_account_companies access where access.account_id::text = p_actor_id and access.company_id = partner_row.company_id and access.is_active = true) then raise exception 'partner_immediate_update_forbidden'; end if;
  for item in select value from jsonb_array_elements(coalesce(p_benefit_items, '[]'::jsonb)) loop
    title := btrim(coalesce(item->>'title', '')); if char_length(title) < 1 or char_length(title) > 500 then raise exception 'partner_immediate_update_invalid_benefit_title'; end if;
    raw_max := nullif(btrim(coalesce(item->>'maxApplyCount', '')), ''); max_count := case when raw_max is null then null else raw_max::integer end; if max_count is not null and max_count < 1 then raise exception 'partner_immediate_update_invalid_benefit_max_apply_count'; end if;
    benefit_id := nullif(item->>'id', '')::uuid; if benefit_id is null then benefit_id := uuid_generate_v4(); end if;
    if exists (select 1 from public.partner_benefits existing where existing.id = benefit_id and existing.partner_id <> partner_row.id) then raise exception 'partner_immediate_update_benefit_forbidden'; end if;
    requested_ids := array_append(requested_ids, benefit_id); benefit_titles := array_append(benefit_titles, title);
    insert into public.partner_benefits (id, partner_id, title, display_order, max_apply_count) values (benefit_id, partner_row.id, title, coalesce((item->>'displayOrder')::integer, 0), max_count) on conflict (id) do update set title = excluded.title, display_order = excluded.display_order, max_apply_count = excluded.max_apply_count, updated_at = now();
  end loop;
  delete from public.partner_benefits where partner_id = partner_row.id and not (id = any(requested_ids));
  if partner_row.thumbnail is not distinct from p_thumbnail and partner_row.images is not distinct from coalesce(p_images, '{}'::text[]) and partner_row.tags is not distinct from coalesce(p_tags, '{}'::text[]) and partner_row.benefit_action_type is not distinct from p_benefit_action_type and partner_row.benefit_action_link is not distinct from p_benefit_action_link and partner_row.reservation_link is not distinct from p_reservation_link and partner_row.inquiry_link is not distinct from p_inquiry_link and partner_row.benefits is not distinct from benefit_titles then raise exception 'partner_immediate_update_no_changes'; end if;
  update public.partners set thumbnail = p_thumbnail, images = coalesce(p_images, '{}'::text[]), tags = coalesce(p_tags, '{}'::text[]), benefit_action_type = p_benefit_action_type, benefit_action_link = p_benefit_action_link, benefits = benefit_titles, reservation_link = p_reservation_link, inquiry_link = p_inquiry_link, updated_at = now() where id = partner_row.id;
  insert into public.admin_audit_logs (request_id, actor_type, actor_id, action, path, target_type, target_id, properties, user_agent, ip_address) values (p_request_id, p_actor_type, p_actor_id, 'partner_portal_immediate_update', p_path, 'partner', partner_row.id::text, coalesce(p_properties, '{}'::jsonb), p_user_agent, p_ip_address);
...
drop trigger if exists platform_activity_from_event_logs on event_logs;
...
alter table platform_active_identities enable row level security;
...
revoke all on table platform_active_identities from anon;
revoke all on table platform_active_identities from authenticated;
...
-- Final migration state: administrator-managed notification template overrides.
...
create table if not exists public.member_email_challenges (
  ...
  attempt_count integer not null default 0,
  created_at timestamp with time zone not null default now(),
  resend_available_at timestamp with time zone not null default now(),
  ...
  constraint member_email_challenges_attempt_count_check
    check (attempt_count between 0 and 10),
  constraint member_email_challenges_delivery_status_check
...
-- Snapshot of 20260715031731_add_scoped_admin_log_rpcs.sql
...
create table if not exists public.mattermost_sender_credentials (
  ...
  constraint mattermost_sender_credentials_health_error_check check (
    health_last_error_code is null
    or health_last_error_code in ('test_target_unavailable', 'unauthorized', 'forbidden', 'rate_limited', 'not_found', 'unavailable', 'timeout', 'invalid_response', 'request_rejected', 'configuration_invalid')
  ),
...
comment on table platform_active_identities is
...
returns table (
  ...
  last_error_code text, health_status text,
  health_checked_at timestamp with time zone, health_failure_count integer,
  health_blocked_until timestamp with time zone, health_last_error_code text,
  expires_at timestamp with time zone,
...
as $$
  select sender.id, sender.generation, sender.status, sender.login_id_hint,
    ...
    sender.last_test_target_kind, sender.last_error_code, sender.health_status,
    sender.health_checked_at, sender.health_failure_count,
    sender.health_blocked_until, sender.health_last_error_code, sender.expires_at,
...
as $$
  ...
  where sender.generation = p_generation and sender.status = 'active'
    and (sender.health_blocked_until is null or sender.health_blocked_until <= now());
$$;
...
as $$
  select sender.id, sender.generation, sender.encrypted_ciphertext,
    ...
    sender.sender_mm_user_id, sender.sender_username_hint
  from public.mattermost_sender_credentials sender
  where sender.status = 'active' and sender.sender_mm_user_id is not null
  order by sender.generation asc;
$$;

create or replace function public.record_mattermost_sender_health_success(p_sender_id uuid)
returns void
language sql
security invoker
set search_path = public, pg_temp
as $$
  update public.mattermost_sender_credentials
  set health_status = 'healthy', health_checked_at = now(), health_failure_count = 0,
      health_blocked_until = null, health_last_error_code = null, updated_at = now()
  where id = p_sender_id and status = 'active';
$$;

create or replace function public.record_mattermost_sender_health_failure(
  p_sender_id uuid, p_error_code text
)
returns void
language plpgsql
security invoker
set search_path = public, pg_temp
as $$
declare next_health_status text; next_blocked_until timestamp with time zone;
begin
  if coalesce(p_error_code, '') not in ('unauthorized', 'forbidden', 'rate_limited', 'unavailable', 'timeout', 'invalid_response', 'request_rejected') then
  ...
  update public.mattermost_sender_credentials
  set health_status = next_health_status, health_checked_at = now(),
      health_failure_count = health_failure_count + 1,
      health_blocked_until = next_blocked_until,
      health_last_error_code = p_error_code, updated_at = now()
  where id = p_sender_id and status = 'active';
end;
$$;
...
-- Final migration state: retain the non-delegable signup approval resource
-- after the older permission-resource migrations have been replayed.
alter table public.admin_permissions
  drop constraint if exists admin_permissions_resource_check;
alter table public.admin_permissions
  add constraint admin_permissions_resource_check
...
-- Final migration state: notification template overrides remain service-role only.
alter table public.graduate_verification_requests
...
where key <> 'super_admin';
-- 20260718202526_add_common_image_upload_sessions.sql
-- Shared private staging for browser image uploads. Final assets remain in
-- their existing feature buckets during the incremental rollout.
create table if not exists public.image_upload_sessions (
  ...
  constraint image_upload_sessions_purpose_check
    check (purpose in ('partner', 'partner-registration', 'partner-change-request', 'review', 'profile', 'graduate-verification', 'manual-member-import', 'promotion')),
  ...
  constraint image_upload_sessions_dimensions_check
    check ((width is null and height is null) or (width between 1 and 10000 and height between 1 and 10000)),
  constraint image_upload_sessions_signed_url_expiry_check
...
create table if not exists public.image_asset_migrations (
  ...
  constraint image_asset_migrations_target_check
    check (char_length(btrim(target_table)) between 1 and 128 and char_length(btrim(target_column)) between 1 and 128 and char_length(btrim(target_row_id)) between 1 and 256),
...
values (
  ...
  false,
  10485760,
  array['image/jpeg', 'image/png', 'image/webp', 'image/avif', 'image/heic', 'image/heif', 'image/heic-sequence', 'image/heif-sequence', 'image/gif', 'image/bmp', 'image/tiff', 'image/svg+xml']
)
on conflict (id) do update set
  public = excluded.public,
  file_size_limit = excluded.file_size_limit,
...
-- 20260718210806_add_manual_import_common_image_upload_ids.sql
alter table public.manual_member_import_rows
...
alter table public.manual_member_import_rows
  ...
  check (
    (photo_filename is null and staging_bucket is null and staging_path is null and photo_content_type is null and photo_size_bytes is null and image_upload_id is null and photo_sha256 is null and photo_width is null and photo_height is null)
    or (image_upload_id is null and photo_filename is not null and staging_bucket = 'manual-member-import-staging' and staging_path is not null and photo_content_type in ('image/jpeg', 'image/png', 'image/webp') and photo_size_bytes between 1 and 5242880 and photo_sha256 is null and photo_width is null and photo_height is null)
    or (image_upload_id is not null and photo_filename is not null and staging_bucket = 'member-profile-images' and staging_path is not null and photo_content_type = 'image/webp' and photo_size_bytes is null and photo_sha256 ~ '^[0-9a-f]{64}$' and photo_width = 640 and photo_height = 640)
  );
...
-- 20260719134505_add_signup_image_upload_policy.sql
-- 신규 회원가입 프로필 이미지는 서명된 signup 세션 소유로만 처리하고,
-- 승인 대기 요청과 연결된 Staging 세션은 최대 7일 보관한다.
  ...
  check (
    ...
    or (
      ...
      and (
        ...
        or (reviewed_by_admin_id is null and rejection_reason = 'approval_timeout')
      )
    )
  );
...
begin
  ...
  where id = request_row.service_policy_document_id
    and kind = 'service'
    and version = request_row.service_policy_version;
  if not found then raise exception 'member_signup_approval_service_policy_missing'; end if;
  select * into privacy_policy
  from public.policy_documents
  where id = request_row.privacy_policy_document_id
    and kind = 'privacy'
    and version = request_row.privacy_policy_version;
  if not found then raise exception 'member_signup_approval_privacy_policy_missing'; end if;
  if request_row.marketing_policy_checked then
    select * into marketing_policy
    from public.policy_documents
    where id = request_row.marketing_policy_document_id
      and kind = 'marketing'
      and version = request_row.marketing_policy_version;
    if not found then raise exception 'member_signup_approval_marketing_policy_missing'; end if;
  end if;
  ...
  insert into public.members (
    ...
    campus, password_hash, password_salt, must_change_password, created_at, updated_at
  ) values (
    request_row.mattermost_account_id,
    btrim(p_display_name),
    p_generation,
  ...
  if request_row.profile_image_upload_id is not null then
    insert into public.member_profile_images (
      member_id, storage_path, sha256, content_type, width, height, source,
      status, reviewed_at, created_at, updated_at
    ) values (
      new_member_id, image_upload_row.final_path, image_upload_row.sha256,
      'image/webp', 640, 640, 'mattermost', 'approved', now(), now(), now()
  ...
  insert into public.member_policy_consents (
    member_id, policy_document_id, kind, version, agreed_at, ip_address, user_agent
  ) values
    (
      new_member_id,
      service_policy.id,
  ...
  insert into public.push_preferences (member_id, marketing_enabled, updated_at)
  values (new_member_id, request_row.marketing_policy_checked, now())
  on conflict (member_id) do update
    set marketing_enabled = excluded.marketing_enabled,
        updated_at = now();
  ...
  where id = request_row.id
    and status = 'pending';
  return jsonb_build_object('status', 'approved', 'member_id', new_member_id);
end;
$$;
...
begin
  ...
  where created_at < cutoff
    and not exists (
      select 1 from public.log_retention_holds hold
...
-- every later index and RPC observes the final token contract.
alter table public.member_password_action_tokens
  drop constraint if exists member_password_action_tokens_purpose_check;
alter table public.member_password_action_tokens
  add constraint member_password_action_tokens_purpose_check
...
-- Snapshot of 20260926203807_showcase_member_multiple_submissions.sql
...
-- Snapshot of 20260905193818_finalize_graduate_cohort_schema.sql
begin;
...
revoke all on function public.complete_member_password_action_with_delivery(text, text, text) from authenticated;
grant execute on function public.complete_member_password_action_with_delivery(text, text, text) to service_role;
-- 20260822205759_fix_admin_initial_setup_reissue.sql
...
-- 20260823144046_add_member_email_resend_cooldown.sql snapshot parity
