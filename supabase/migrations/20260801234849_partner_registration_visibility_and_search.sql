-- Persist the reviewer's requested visibility and support server-side queue search.
-- Rollback: restore the previous four-argument RPC and drop visibility after
-- migrating any non-public request values back to public.

alter table public.partner_registration_requests
  add column if not exists visibility text not null default 'public';

alter table public.partner_registration_requests
  drop constraint if exists partner_registration_requests_visibility_check;

alter table public.partner_registration_requests
  add constraint partner_registration_requests_visibility_check
...
create index if not exists partner_registration_requests_visibility_created_idx
  on public.partner_registration_requests(visibility, created_at desc, id desc);

drop function if exists public.get_admin_partner_registration_request_page(text, integer, integer, text[]);
  ...
  scoped_rows as (
    ...
    where (parameters.status_filter is null or request.status = parameters.status_filter)
      ...
      and (
        ...
        or (
          company.id is null
          and request.location ~ '(전국|전 지점|전체 지점|모든 지점|전 매장|전체 매장|모든 매장|서울|강남|역삼|역삼역|선릉|테헤란|봉은사|논현|구미|경북|경상북도|대전|유성|둔산|부산|울산|경남|창원|김해|양산|해운대|서면|광주|전남)'
