alter table public.ad_coupons
  add column if not exists per_member_daily_issue_limit integer,
  add column if not exists per_member_weekly_issue_limit integer,
  add column if not exists per_member_monthly_issue_limit integer,
  add column if not exists onsite_password_hash text,
  add column if not exists onsite_password_salt text;

alter table public.ad_coupons
  drop constraint if exists ad_coupons_per_member_issue_limit_check;
alter table public.ad_coupons
  add constraint ad_coupons_per_member_issue_limit_check
  check (
...
alter table public.ad_coupons
  drop constraint if exists ad_coupons_onsite_password_check;
alter table public.ad_coupons
  add constraint ad_coupons_onsite_password_check
  check (
...
alter table public.ad_coupon_issues
  add column if not exists onsite_password_hash_snapshot text,
  add column if not exists onsite_password_salt_snapshot text;

alter table public.ad_coupon_issues
  drop constraint if exists ad_coupon_issues_onsite_password_snapshot_check;
alter table public.ad_coupon_issues
  add constraint ad_coupon_issues_onsite_password_snapshot_check
  check (
...
create index if not exists ad_coupon_issues_member_coupon_issued_idx
  on public.ad_coupon_issues(coupon_id, member_id, issued_at desc);
...
drop function if exists public.redeem_ad_coupon_issue(uuid, uuid, text, jsonb);
