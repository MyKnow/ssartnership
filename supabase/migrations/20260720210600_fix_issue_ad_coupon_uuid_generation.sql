-- Coupon issuance must not depend on uuid-ossp being visible through the
-- function search_path. Supabase projects expose pgcrypto's UUID generator,
-- which is also available on installations where uuid_generate_v4() is not.
