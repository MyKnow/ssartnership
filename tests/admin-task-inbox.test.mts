...
test("작업함의 DB 실패는 안전한 미확인 상태로 표현한다", async () => {
  ...
  assert.equal(toSafeAdminTaskQueueCount({ count: 11, error: { message: "internal" } }), null);
});
...
test("작업함 집계 RPC는 권한 없는 큐를 계산하지 않고 service role로만 실행된다", async () => {
  ...
  assert.match(migration, /company\.managed_campus_slugs && scope\.managed_campus_slugs/);
...
test("홈의 다음 작업은 실제 대기 건이 있는 권한 내 작업만 선택한다", async () => {
  ...
  const tasks = [
    { href: "/admin/partner-registrations?status=pending", label: "신규 제휴 접수" },
...
test("관리 홈은 다음 작업을 우선 표시하고, 활동 지표 조회가 첫 화면을 막지 않는다", async () => {
  ...
  assert.doesNotMatch(pageSource, /fetchAdminPlatformActivityMetrics\(supabase\)/);
