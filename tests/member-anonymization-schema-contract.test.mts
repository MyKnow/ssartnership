...
test("member anonymization only updates current members columns", () => {
  const schemaSql = readRepoFile("supabase/schema.sql");
  const body = extractFunctionBody(
    extractLatestAnonymizationContract(schemaSql),
  );
...
test("member anonymization preserves the retention gate and current cleanup contract", () => {
  const schemaSql = readRepoFile("supabase/schema.sql");
  const contract = extractLatestAnonymizationContract(schemaSql);
...
test("member anonymization retains restrictive function privileges", () => {
  const schemaSql = readRepoFile("supabase/schema.sql");
