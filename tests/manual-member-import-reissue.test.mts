...
test("전송 결과 미확인 행은 관리자 확인 뒤에만 새 초기 설정 링크를 발급한다", async () => {
  ...
  assert.match(service, /select\("id,display_name,mattermost_account_id,email_normalized,generation,staff_source_generation,must_change_password"\)/);
  ...
  assert.match(service, /withActiveMattermostSenderForGeneration\(\s*mattermostRecipient\.senderGeneration/);
  assert.match(service, /session\.sendDirectMessage\(\s*mattermostRecipient\.mattermostUserId/);
