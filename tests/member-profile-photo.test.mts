...
test("인증 카드의 private 사진 API도 승인 상태만 읽는다", async () => {
  const route = await readFile(profileImageRoutePath, "utf8");

  assert.match(route, /profile_photo_review_status/);
  assert.match(route, /member\?\.profile_photo_review_status !== "approved"/);
});
