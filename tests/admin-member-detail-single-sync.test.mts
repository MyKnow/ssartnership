...
test("백필도 사진 미처리를 변경 없음으로 집계하지 않는다", async () => {
  ...
  assert.match(memberSync, /if \(result\.syncResult\.imageSkipped\) \{\s*photoSkipped\.push\(result\.syncResult\);/);
  assert.match(memberSync, /if \(!result\.syncResult\.updated && !result\.syncResult\.imageSkipped\) \{\s*skipped \+= 1;/);
