...
test("관리자 개인 알림은 서버 read-model과 안전한 복구 상태를 사용한다", async () => {
  ...
  assert.match(readModelSource, /ADMIN_NOTIFICATION_SETTINGS_CACHE_REVALIDATE_SECONDS/);
  assert.match(readModelSource, /admin-notification-preferences/);
  assert.match(readModelSource, /admin-notification-device-count/);
  assert.match(readModelSource, /invalidateAdminNotificationSettingsCache/);
