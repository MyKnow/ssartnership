...
test("활성 사용자 집계는 회원과 비로그인 세션을 분리하고 재실행 가능한 로그 백필을 제공한다", () => {
  ...
  const forwardMetricsMigration = migrationNames.find((name) =>
    name.endsWith("_add_admin_forward_activity_metrics.sql"),
  );
  assert.ok(forwardMetricsMigration, "전방 활성 지표 migration이 필요합니다.");
  assert.match(
    readMigration(forwardMetricsMigration),
    /order\s+by\s+day_series\.activity_date\)\s+from\s+day_series\s*\),\s*'\[\]'::jsonb\)\s+as\s+daily_series/i,
  );
});
