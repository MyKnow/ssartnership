...
test("수료생 계정과 탈퇴 식별자 예약은 정규화 인증 관계만 사용한다", () => {
  ...
  assert.match(memberLifecycle, /from\("member_ssafy_verifications"\)/);
  assert.doesNotMatch(
    memberLifecycle,
    /\.select\("id,email_normalized,mattermost_account_id,mm_user_id,mm_username,ssafy_sub"|identity\.mm_user_id|identity\.mm_username|identity\.ssafy_sub/,
  );
