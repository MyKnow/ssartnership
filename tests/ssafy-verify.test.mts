...
test("SSAFY Verify signup profile falls back to verified claims when profile omits Mattermost fields", async () => {
  const originalEnv = {
    issuer: process.env.SSAFY_VERIFY_ISSUER,
    serverClientId: process.env.SSAFY_VERIFY_SERVER_CLIENT_ID,
    serverClientSecret: process.env.SSAFY_VERIFY_SERVER_CLIENT_SECRET,
  ...
  try {
    ...
    globalThis.fetch = (async (input) => {
      ...
      assert.equal(
        url,
        "https://verify.example.com/v1/mattermost-users/mm.user-123/profile",
      );
      return new Response(
        JSON.stringify({
          ok: true,
          request_id: "icn1::profile-minimal",
          data: {
            sub: "pairwise-subject",
            updated_at: "2026-06-22T04:10:00.000Z",
          },
        }),
        { status: 200, headers: { "content-type": "application/json" } },
      );
    ...
    assert.deepEqual(result, {
      ...
      session: {
        sub: "pairwise-subject",
        mattermostUserId: "mm.user-123",
        mattermostUsername: "mm.user-123",
        displayName: "김싸피",
        cohort: 15,
        campus: "서울",
        isStaff: false,
