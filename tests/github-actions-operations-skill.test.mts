...
test("trusted dependency installation disables every lifecycle and verifies one pinned binary", async () => {
  ...
  const controlledEnvironment = policyModule.buildControlledInstallEnvironment({
    ...
    EXTERNAL_PAYMENT_PRIVATE_KEY_BASE64: "synthetic-payment-secret",
  ...
  for (const secretKey of [
    ...
    "EXTERNAL_PAYMENT_PRIVATE_KEY_BASE64",
    "GITHUB_TOKEN",
    "NODE_AUTH_TOKEN",
    "NODE_OPTIONS",
  ...
  const report = auditor.buildSafeReport({
    ...
    rawAnnotations: [{
      ...
      annotation: {
        ...
        title: `EXTERNAL_PAYMENT_PRIVATE_KEY_BASE64=${hostileValues[0]}`,
    ...
    log:
      `##[error] EXTERNAL_PAYMENT_PRIVATE_KEY_BASE64=${hostileValues[0]}\n`
