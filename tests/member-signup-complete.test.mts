...
test("회원가입 완료 API는 검증 세션을 사용하고 Mattermost 재조회에 의존하지 않는다", async () => {
  const route = await readFile(
    new URL("../src/app/api/mm/signup/route.ts", import.meta.url),
    "utf8",
  );

  assert.match(route, /verification\.mmUserId/);
  assert.match(route, /verification\.mmUsername/);
  assert.match(route, /verification\.displayName/);
  assert.match(route, /verification\.signupMode === "approval"/);
  assert.doesNotMatch(route, /withActiveMattermostSenderForGeneration/);
...
test("회원가입 프로필 이미지 연결은 Storage 변환 경로와 ledger 저장 오류를 구분한다", async () => {
  const [profile, repository] = await Promise.all([
    readFile(new URL("../src/lib/member-signup-profile.ts", import.meta.url), "utf8"),
    readFile(new URL("../src/lib/image-upload/repository.supabase.ts", import.meta.url), "utf8"),
  ]);

  assert.match(repository, /profile_image_staging_integrity/);
  assert.match(profile, /profile_image_ledger_lookup/);
  assert.match(profile, /profile_image_ledger_insert/);
});
