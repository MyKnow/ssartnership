...
const steps = [
  ...
  ...(process.env.CI_EXECUTION_PROFILE === "mac-amd64"
...
writeFileSync("/work/.self-host-build/gate.json", `${JSON.stringify({ ...totals, retries: 0, video: "off", compiledSource: "single-deployable-build", deployableFingerprint, e2eRuntime: process.env.CI_EXECUTION_PROFILE === "mac-amd64" ? "production-test-only" : "development", fixtureBuildDeployable: false })}\n`, { flag: "wx", mode: 0o600 });
