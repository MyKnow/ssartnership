...
export default async function AdminCycleMockPage() {
  ...
  return <AdminShell title="전체 인증 카드 목업" backHref="/admin/cycle" backLabel="기수 관리"><div className="grid gap-6"><AdminPageHeader eyebrow="기수 목업" title="전체 목업보기" description="운영진과 색상이 지정된 모든 기수의 인증 카드를 슈퍼 어드민 프로필로 확인합니다." /><AdminAllCertificationCardMocks themes={themes} member={member} initialTimestamp={new Date().toISOString()} /></div></AdminShell>;
}
