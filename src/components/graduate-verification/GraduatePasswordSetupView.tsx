...
export default function GraduatePasswordSetupView({
  initialToken,
}: {
  /** Story/test-only input. Real email links use location.hash. */
  initialToken?: string;
}) {
  const router = useRouter();
  const [token, setToken] = useState(initialToken ?? "");
  const [isTokenReady, setIsTokenReady] = useState(Boolean(initialToken));
  ...
  useEffect(() => {
    ...
    const fragment = window.location.hash.startsWith("#")
      ...
      : "";
    const oneTimeToken = new URLSearchParams(fragment).get("token")?.trim() ?? "";
    if (oneTimeToken) {
      ...
      setMessage("비밀번호 설정 링크가 없거나 이미 사용되었습니다. 새 링크를 요청해 주세요.");
    }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage(null);
    try {
      const response = await fetch("/api/graduate-verification/account/setup", {
        method: "POST",
  ...
  return (
    <Card className="mx-auto max-w-lg">
      <h1 className="text-ko-title text-2xl font-semibold">비밀번호 설정</h1>
      <p className="text-ko-pretty mt-2 text-sm text-muted-foreground">수료생 이메일 계정에 사용할 새 비밀번호를 설정해 주세요.</p>
      <form className="mt-6 grid gap-4" onSubmit={submit} noValidate>
        ...
        <Button type="submit" disabled={!isTokenReady || !token} loading={pending} loadingText="설정 중">비밀번호 설정 완료</Button>
      </form>
    </Card>
  );
}
