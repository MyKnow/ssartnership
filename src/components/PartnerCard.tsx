...
const categoryTone: Record<Category["key"], string> = {
    ...
    <article className="flex h-full flex-col justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex flex-col gap-4">
        <div className="flex items-start justify-between gap-3">
          <span
            className={`rounded-full px-3 py-1 text-xs font-semibold ${categoryTone[partner.category]}`}
          >
            {categoryLabel}
          </span>
        ...
        <div className="text-sm text-slate-700">
          <p className="font-medium text-slate-800">연락처</p>
          <p className="mt-1">{partner.contact}</p>
        </div>
        <div className="text-sm text-slate-700">
          <p className="font-medium text-slate-800">혜택</p>
      ...
      <div className="mt-5 flex items-center justify-between">
        <p className="text-xs text-slate-500">제휴 문의는 운영진에게 전달</p>
        {partner.mapUrl ? (
          <a
