  ...
  return (
    <div className="grid min-w-0 gap-8">
      <AdminReviewQueueHeader
        eyebrow="Graduate verification"
        title="수료생 인증 검토"
        description="신규 수료생과 기존 회원 복구 요청의 증빙을 확인하고, 다음 상태로 안전하게 전환합니다."
        metrics={[
          { label: "검토 대기", value: `${requests.length}건`, hint: "현재 큐에 있는 요청" },
          { label: "신규 접수", value: `${submittedCount}건`, hint: "아직 검토하지 않은 요청" },
          { label: "검토 중", value: `${inReviewCount}건`, hint: "관리자가 확인 중인 요청" },
          { label: "메일 재발송", value: `${setupEmailRetries.length}건`, hint: "설정 메일 실패 건" },
        ]}
        feedback={feedback}
        nextAction={{
