...
export type AdminReviewFormAction = (formData: FormData) => void | Promise<void>;
    ...
    <Card padding="md" className="grid gap-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="grid gap-3">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={review.isHidden ? "warning" : "success"}>{statusLabel}</Badge>
            ...
            <span className="text-xs text-muted-foreground">{formatPartnerReviewDate(review.createdAt)}</span>
          </div>
          ...
          <div className="grid gap-1">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>{review.authorMaskedName} · {review.authorRoleLabel}</span>
            </div>
          </div>
        </div>
      ...
      {editable ? (
        <details className="rounded-xl border border-border bg-surface-muted/60 p-3">
          <summary className="cursor-pointer text-sm font-semibold text-foreground">리뷰 수정</summary>
          <form action={updateAction} className="mt-3 grid gap-3">
            ...
            <div className="flex justify-end">
              <SubmitButton variant="secondary" pendingText="수정 중">리뷰 수정</SubmitButton>
            </div>
          </form>
        </details>
      ) : null}
