...
export default function AdminAdPackageManager({
  ...
  const couponCount = campaigns.reduce(
    (sum, campaign) => sum + campaign.coupons.length,
    0,
  );
    ...
    <section className="grid gap-5">
      ...
      <div className="grid gap-4">
        <Card tone="elevated" className="grid gap-4">
          <SectionHeading
            title={canCreate ? "캠페인 생성" : "캠페인 생성 권한 없음"}
            description={
              canCreate
                ? "제휴처와 패키지를 연결하고 홈 배너/푸시/쿠폰 채널을 선택합니다."
                : "현재 계정은 캠페인 정보를 조회할 수 있지만 새 캠페인을 만들 수 없습니다."
            }
          />
          {canCreate ? (
                ...
                <StatsRow
                  items={[
                    ...
                    },
                    {
                      label: "배너 클릭",
                      value:
                        campaign.metrics.homeBannerClicks.toLocaleString(
                          "ko-KR",
                        ),
                      hint: "홈 스폰서 배너",
                    },
                    {
                      label: "쿠폰 관심",
                      value:
                        campaign.metrics.couponIntentCount.toLocaleString(
                          "ko-KR",
                        ),
                    ...
                    },
                    {
                      label: "쿠폰 사용",
                      value:
                        campaign.metrics.couponRedemptions.toLocaleString(
                          "ko-KR",
                        ),
