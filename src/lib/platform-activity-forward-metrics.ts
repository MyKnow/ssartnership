...
export async function fetchForwardActivityMetrics(anchorDate?: string) {
  try {
    ...
    return {
      metrics: {
        asOfDate: date(row.as_of_date),
        todayDate: date(row.today_date),
        memberDau: count(row.member_dau),
        memberWau: count(row.member_wau),
        memberMau: count(row.member_mau),
        wauObservedThrough: date(row.wau_observed_through),
        mauObservedThrough: date(row.mau_observed_through),
        dailySeries: series.flatMap((item) => {
          if (!item || typeof item !== "object") return [];
          const point = item as Record<string, unknown>;
          const pointDate = date(point.activity_date);
          const wauThrough = date(point.wau_observed_through);
          const mauThrough = date(point.mau_observed_through);
          return pointDate && wauThrough && mauThrough
            ? [
                {
                  date: pointDate,
                  memberActiveCount: count(point.member_active_count),
                  guestSessionCount: count(point.guest_session_count),
                  memberWau: count(point.member_wau),
                  memberMau: count(point.member_mau),
                  wauObservedThrough: wauThrough,
                  mauObservedThrough: mauThrough,
                },
              ]
            : [];
        }),
...
export function emptyForwardActivityMetrics(): ForwardActivityMetrics {
  return {
    ...
    memberWau: 0,
    memberMau: 0,
    wauObservedThrough: null,
    mauObservedThrough: null,
    dailySeries: [],
  };
}
