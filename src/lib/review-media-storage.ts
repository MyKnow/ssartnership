...
export async function uploadReviewMediaFile(
  ...
  const storagePath = `reviews/${partnerId.trim()}/${reviewId.trim()}/${index}-${randomUUID()}.webp`;
  ...
  const { data } = supabase.storage.from(REVIEW_MEDIA_BUCKET).getPublicUrl(storagePath);
