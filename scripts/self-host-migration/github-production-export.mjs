...
import { exportStorage, PRODUCTION_PROJECT } from "./storage.mjs";
import { sealMigrationFile } from "./transfer.mjs";
import { SUPABASE_POSTGRES_DUMP_IMAGE } from "../supabase-sync-preview-dump-lib.mjs";

export const EXPORT_BRANCH = "feat/self-host-production-20260909";
export const EXPORT_WORKFLOW = ".github/workflows/self-host-production-export.yml";
...
export function exportContext(env = process.env) {
  ...
  return { sourceProject: PRODUCTION_PROJECT, sha: env.GITHUB_SHA, runId: Number(env.GITHUB_RUN_ID) };
}
export function validateExportRequest(value, now = Date.now()) {
  if (!value || Object.keys(value).sort().join() !== "expiresAt,sourceProject,version" || value.version !== 1 || value.sourceProject !== PRODUCTION_PROJECT
...
export function validateProductionApi(url, key) {
  if (url !== `https://${PRODUCTION_PROJECT}.supabase.co` || typeof key !== "string" || key.length > 4096 || !/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/u.test(key)) fail("API_IDENTITY_INVALID");
  try {
    const payload = JSON.parse(Buffer.from(key.split(".")[1], "base64url").toString("utf8"));
    if (payload.ref !== PRODUCTION_PROJECT || payload.role !== "service_role") fail("API_IDENTITY_INVALID");
...
async function validateSource(context) {
  ...
  validateExportRequest(await json(path.join(root, "deploy/self-host-migration/production-export-request.json")));
...
async function prepare(context) {
  ...
  const toolsDirectory = path.join(homedir(), "ssartnership-production-migration-age");
...
async function capture(context) {
  await validateSource(context);
  validateProductionApi(process.env.SUPABASE_PRODUCTION_URL, process.env.SUPABASE_PRODUCTION_SERVICE_ROLE_KEY);
  const planFor = cloudDatabaseClient(process.env.SUPABASE_PRODUCTION_DB_URL, PRODUCTION_PROJECT);
  const publicKey = await json(path.join(root, "deploy/self-host-migration/production-recipient.json"));
  if (publicKey.version !== 1 || publicKey.sourceProject !== PRODUCTION_PROJECT) fail("RECIPIENT_INVALID");
  ...
  const directory = path.join(temporary, "production-cloud-export"); await mkdir(directory, { mode: 0o700 });
  const payload = path.join(directory, "payload"); await mkdir(payload, { mode: 0o700 });
  console.log('{"stage":"database-capture"}');
  const database = await captureDatabase(path.join(payload, "database"), planFor, PRODUCTION_PROJECT);
  console.log('{"stage":"storage-capture"}');
  let reads = 0;
  const storage = await exportStorage({ directory: path.join(payload, "storage"), serviceKey: process.env.SUPABASE_PRODUCTION_SERVICE_ROLE_KEY, sourceProject: PRODUCTION_PROJECT, concurrency: 4,
